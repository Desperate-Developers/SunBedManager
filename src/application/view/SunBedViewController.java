...
public class SunBedViewController implements Initializable
{
	...
	public void initialize(URL arg0, ResourceBundle arg1)
	{
		...
		endDayBtn.addEventHandler(MouseEvent.MOUSE_CLICKED, new EventHandler<MouseEvent>(){
		       ...
		       public void handle(MouseEvent e){
		    	     ...
		    	     alert.setContentText("DO YOU WANT TO CLEAR THE SUNBEDS?");
		...
		if(SunBedCollection.getInstance().getCount() > 0) {
			startTab.setDisable(false);
			this.startAnchorPane.setStyle("-fx-background-color: linear-gradient(gray, white, white, gray)") ;
			this.useBtn.setId("submit_btn_style") ;
			this.clearBtn.setId("submit_btn_style") ;
	...
	public void submit()
	{
		if(!textField.getText().isBlank())
		{
			...
			Alert alert = new Alert(AlertType.CONFIRMATION);
			alert.setContentText("DO YOU WANT TO SUBMIT THE SUNBEDS?");
			Optional<ButtonType> result = alert.showAndWait();
	...
	@FXML
	private void handleUseBtn() {
		this.numOfSunBeds = SunBedCollection.getInstance().getCount();
		System.out.println("Using sunbeds: " + this.numOfSunBeds);
		drawSunBeds() ;
		this.tabPane.getSelectionModel().select(sunbedsTab) ;
	...
	private void handleClearBtn() {
		System.out.println("Clearing sunbeds");
